...
const makeSixteen = () => {
	const str = 'JavaScript';
	resultSixteen.innerText = str.substring(3);
};
...
const makeEighteen = () => {
	...
	resultEighteen.innerText = str.replace(/world/g,'JavaScript');
};
...
const makeTwentyThree = () => {
	let a = 2;
	let b = 5;
	resultTwentyThree.textContent = Math.pow(a,b); 
};
...
const makeTwentySix = () => {
	...
	resultTwentySix.textContent = randomNum.toFixed(4);
};
...
const makeTwentySeven = () => {
	...
	resultTwentySeven.textContent = Math.floor(Math.random() * (max - min + 1)) + min; //Максимум и минимум включаются
};
...
const makeTwentyNine = () => {
	resultTwentyNine.innerHTML = Math.sqrt(16, 2); 
};
...
const makeThirty = () => {
	resultThirty.innerHTML = Math.sqrt(25, 2); 
};
